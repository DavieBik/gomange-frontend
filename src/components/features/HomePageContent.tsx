...
import type { Restaurant, Collection } from '@/types/sanity';
...
export default function HomePageContent() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true)
      const [restaurants, cols] = await Promise.all([
        fetchRestaurants(),
        fetchCollections()
      ])
          ...
          <div className="flex gap-6 sm:gap-8 pb-2 hide-scrollbar overflow-x-auto">
            {previewRestaurants.map((restaurant) => (
              <div key={restaurant._id} className="flex-shrink-0">
                {/* Carrusel */}
                <div className="relative z-10 bg-white rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl min-w-[260px] max-w-[260px] h-[370px] flex flex-col mx-1">
                  <RestaurantCard restaurant={restaurant} layout="carousel" />
                </div>
              </div>
            ))}
          </div>
